...
function getPP(data, frcd) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (
            q.verdict == "OK" &&
            q.problem.rating != undefined &&
            calc[p] == undefined &&
            (!frcd ||
                q.author.participantType == "CONTESTANT" ||
                q.author.participantType == "OUT_OF_COMPETITION")
        )
...
function getACCounts(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        var q = data[i];
        var p = q.problem.contestId + q.problem.index;
        if (q.verdict == "OK" && calc[p] == undefined) ++ans, (calc[p] = true);
    }
    delete calc;
    return ans;
}
...
function getProblemStatistics(d) {
    ...
    for (var i = 0; i < d.length; i++) {
        for (var j = 0; j < d[i].tags.length; j++) {
            if (app[d[i].tags[j]] == undefined) app[d[i].tags[j]] = 0;
            ++app[d[i].tags[j]];
        }
    }
    var lis = [];
    for (var x in app) if (app.hasOwnProperty(x)) lis.push([x, app[x]]);
    ...
    // rating analyse
    var lis2 = [];
    for (var i = 800; i <= 3500; i += 100) lis2.push(0);
    for (var i = 0; i < d.length; i++)
        if (d[i].rating != undefined) ++lis2[(d[i].rating - 800) / 100];
    ret.push(lis2);
    return ret;
}
...
function problemPassedIf(id) {
    ...
    for (var x = 0; x < i.length; x++) {
        var t = i[x];
        if (t.verdict == "OK") return (profileCalculatedProblemType[id] = true);
    }
...
function problemSolvedInContestIf(id) {
    ...
    if (problemPassedIf(id)) {
        for (var x = 0; x < i.length; x++) {
            ...
            if (
                t.verdict == "OK" &&
                (t.author.participantType == "CONTESTANT" ||
                    t.author.participantType == "OUT_OF_COMPETITION")
            )
...
var profileListFilterFuncs = [
    ...
    function (q, argv) {
        ...
        if (t.length < T.length) return false;
        return t.substring(0, T.length) == T;
    },
    function (q, argv) {
        var t = (q.contestId + q.index).toLowerCase();
        var T = argv[1].toLowerCase();
        if (T.length <= String(q.contestId).length) return false;
        return t.substring(0, T.length) == T;
    },
];
...
function profileProblemFilter(d, mn, mx, ft) {
    ...
    if (mn != 0) {
        ...
        for (var i = 0; i < q.length; i++)
            if (q[i].rating != undefined && q[i].rating >= mn) ret.push(q[i]);
    }
    if (mx != 0) {
        var q = ret;
        ret = [];
        for (var i = 0; i < q.length; i++)
            if (q[i].rating != undefined && q[i].rating <= mx) ret.push(q[i]);
    }
    for (var j = 0; j < profileChosenTags.length; j++) {
        ...
        ret = [];
        for (var i = 0; i < q.length; i++)
            if (q[i].tags.indexOf(profileIdToTag[tg]) != -1) ret.push(q[i]);
    }
    ft = ft.split(" ");
    var namT = [],
        tagT = [];
...
function profileProblemSort(d, st, dir) {
    function getVal(q, s) {
        if (s == 1) return [q.contestId, q.index];
        if (s == 2) return [q.rating == undefined ? 0 : Number(q.rating)];
        return [q.name];
    }
...
function profileDisplayAllSubmissions(pid) {
    ...
    for (var i = 0; i < useful.length; i++) {
        var tim = "";
        var curr = useful[i];
        tim = new Date(curr.creationTimeSeconds * 1000).pattern(
            "yyyy/MM/dd hh:mm"
        );
        var vid = "";
        if (curr.verdict == "OK")
            vid = `<span class="green" style="font-weight: bold">${toDetailedInfo(
                curr.verdict,
                curr.testset
            )}</span>`;
            ...
            vid = `<span>${toDetailedInfo(
                curr.verdict,
                curr.testset
            )} on test ${curr.passedTestCount + 1}</span>`;
        else if (
            curr.verdict == "PARTIAL" ||
            curr.verdict == "COMPILATION_ERROR" ||
            curr.verdict == "SKIPPED" ||
            curr.verdict == "REJECTED"
        )
            vid = `<span class="red">${toDetailedInfo(
                curr.verdict,
                curr.testset
            )}</span>`;
        else if (curr.verdict == "CHALLENGED")
            vid = `<span class="red" style="font-weight: bold">${toDetailedInfo(
                curr.verdict,
                curr.testset
            )}</span>`;
        else
        ...
        $(".eventList").append(
            `<p>${tim} ${vid}${
                curr.points != undefined || curr.pointsInfo != undefined
                    ? ` | ${
                          curr.pointsInfo != undefined
                              ? curr.pointsInfo
                              : curr.points
                      }`
                    : ""
            } [${toDetailedTestset(curr.testset)}]</p>`
        );
    }
}
...
function renderProblemMain() {
    ...
    var len = Math.floor(
        (profileFilteredInfo.length + profilePageCount - 1) / profilePageCount
    );
    len = Math.max(len, 1);
    ...
    for (var i = l; i < r; i++) {
        ...
        for (var j = 0; j < profileFilteredInfo[i].tags.length; j++)
            tgs += `<span class="profileTagBox" force="true" tagId="${
                profileTagToId[profileFilteredInfo[i].tags[j]]
            }" style='background: ${
                problemTagsToColor[profileFilteredInfo[i].tags[j]][1]
            };'>${profileFilteredInfo[i].tags[j]}</span>`;
        if (tgs == "")
            tgs = `<span class="profileTagBox" style='background: rgba(127, 127, 127, 0.5);'>-</span>`;
        $(".profileProblemMain").append(
            `<div class='profileProblemBlock'><div style='width: 20px; text-align: center'><i class='${
                problemPassedIf(
                    profileFilteredInfo[i].contestId +
                        profileFilteredInfo[i].index
                )
                    ? "fas fa-check green"
                    : "fas fa-times red"
            }' style="cursor: pointer" onclick="profileDisplayAllSubmissions('${
                profileFilteredInfo[i].contestId + profileFilteredInfo[i].index
            }')"></i></div><div style="width: 100px">${
                profileFilteredInfo[i].contestId + profileFilteredInfo[i].index
            }</div><div style="flex: 1"><span onclick="openProblemWin(['${
                profileFilteredInfo[i].contestId + profileFilteredInfo[i].index
            }'])" style="cursor: pointer">${
                profileFilteredInfo[i].name
            }</span><span style='color: grey; font-size: 12px' class='profileProblemRating'> *${
                profileFilteredInfo[i].rating == undefined
                    ? "?"
                    : profileFilteredInfo[i].rating
            }</span></div></div>`
        );
        $(".profileProblemMain").append(
            `<div class='profileProblemTags'>${tgs}</div>`
        );
    }
}
$(".profilePageLeft").click(function () {
    if (profileFilteredInfo == undefined) return;
    --profilePageID;
    renderProblemMain();
});
...
function renderProfileProblemsPage(d, fir) {
    ...
    var Sm = 0;
    for (var i = 0; i < D[0].length; i++) Sm += D[0][i][1];
    $(".profileTagsDisplayer").append(
        `<span class="profileTagBox" style='background: rgba(127, 127, 127, 0.5);'>all: ${d.length}</span>`
    );
    for (var i = 0; i < profileChosenTags.length; i++) {
        ...
        $(".profileTagsBar").append(
            `<div class='problemTagsBarContent' style="background-color: ${
                problemTagsToColor[tg][0]
            }; width: ${
                Sm == 0 ? 0 : (D[0][i][1] / Sm) * 100
            }%" title='${tg}: ${ps}'></span>`
        );
        $(".profileTagsDisplayer").append(
            `<span class="profileTagBox" tagId="${profileTagToId[tg]}" style='background: ${problemTagsToColor[tg][1]};'><i class='fas fa-circle' style="color: ${problemTagsToColor[tg][0]}"></i> ${tg}: ${ps}</span>`
        );
    }
    if (D[0].length) {
        for (var i = 0; i < D[0].length; i++)
            if (profileChosenTags.indexOf(profileTagToId[D[0][i][0]]) == -1) {
                ...
                $(".profileTagsBar").append(
                    `<div class='problemTagsBarContent' style="background-color: ${
                        problemTagsToColor[D[0][i][0]][0]
                    }; width: ${(D[0][i][1] / Sm) * 100}%" title='${
                        D[0][i][0]
                    }: ${D[0][i][1]}'></span>`
                );
                $(".profileTagsDisplayer").append(
                    `<span class="profileTagBox" tagId="${
                        profileTagToId[D[0][i][0]]
                    }" style='background: ${
                        problemTagsToColor[D[0][i][0]][1]
                    };'><i class='far fa-circle' style="color: ${
                        problemTagsToColor[D[0][i][0]][0]
                    }"></i> ${D[0][i][0]}: ${D[0][i][1]}</span>`
                );
            }
    }
    var mx = 0;
    for (var i = 0; i < D[1].length; i++) mx = Math.max(mx, D[1][i]);
    for (var i = 0; i < D[1].length; i++) {
        var currRating = i * 100 + 800;
        var currPassed = D[1][i];
        var prob = 0;
        if (mx != 0) prob = currPassed / mx;
        $(".profileRatingGraphBar" + currRating).css("height", 60 * prob);
        $(".profileRatingGraphDesc" + currRating)
            .css("bottom", 60 * prob + 30)
            .html(currPassed);
    }
    // cssController2
    profilePageID = 1;
    ...
    $(".profileTagBox")
        ...
        .click(function () {
            ...
            q = Number(q);
            if (
                profileChosenTags.indexOf(q) != -1 &&
                !($(this).attr("force") == "true")
            )
...
$(".profileTagsDisplayIf").click(function () {
    if ($(this).hasClass("selected")) {
        ...
        $(".cssController2").html(
            ".profileProblemTags, .profileTagBox[tagId], .profileTagsBar{display: none;} .profileTagsDisplayer{height: 30px; margin-top: 10px} .profileProblemBlock{margin: 15px 0px}"
        );
    } else {
        $(this).addClass("selected");
        $(".cssController2").html(
            ".profileProblemTags{display: block;} .profileTagsBar{display: flex;} .profileTagBox[tagId]{display: inline-block}"
        );
    }
});
...
$(".profileRatingDisplayIf").click(function () {
    if ($(this).hasClass("selected")) {
        $(this).removeClass("selected");
        $(".cssController4").html(
            ".profileProblemRating{display: none} .profileRatingGraph{display: none} .profileProblemMain{margin-top: 10px}"
        );
    } else {
        $(this).addClass("selected");
        $(".cssController4").html(
            ".profileProblemRating{display: inline} .profileRatingGraph{display: block}"
        );
    }
});
...
function initProfileRatingGraph() {
    ...
    for (var i = 800; i <= 3500; i += 100) {
        ...
        if ((i - 800) % 3 == 0)
            $(".profileRatingGraph").append(
                `<span style='transform: translate(-50%, 0); position: absolute; bottom: 10px; left: ${
                    (((i - 800) / 100 + 0.5) / 28) * wd + 10
                }px; color: grey; font-size: 12px'>${currRating}</span>`
            );
        $(".profileRatingGraph").append(
            `<div number=${i} class="profileRatingGraphBarContent profileRatingGraphBar${i}" style='cursor: pointer; transition: 0.3s; border-radius: 5px; transform: translate(-50%, 0); position: absolute; bottom: 30px; left: ${
                (((i - 800) / 100 + 0.5) / 28) * wd + 10
            }px; background-color: ${col}; width: 10px; height: ${
                60 * prob
            }px'></div>`
        );
        $(".profileRatingGraph").append(
            `<span number=${i} class="profileRatingGraphDescContent profileRatingGraphDesc${i}" style='cursor: pointer; transition: 0.3s; transform: translate(-50%, 0); position: absolute; bottom: ${
                60 * prob + 30
            }px; left: ${
                (((i - 800) / 100 + 0.5) / 28) * wd + 10
            }px; color: grey; font-size: 12px'>${0}</span>`
        );
    }
...
function reloadProfileProblemPage(fir) {
    ...
    var st = 0;
    if ($(".profileSelectSortID").hasClass("selected")) st = 1;
    else if ($(".profileSelectSortRating").hasClass("selected")) st = 2;
    else st = 3;
    var dir = 0;
    if ($(".profileSelectSortDirection i").hasClass("fa-caret-down")) dir = 1;
...
$(".profileSelectSortDirection").click(function () {
    ...
    else $(this).find("i").attr("class", "fas fa-caret-down green");
    if (profileFilteredInfo == undefined) return;
    reloadProfileProblemPage(false);
});
...
function profileDrawGraph(data) {
    if (profileRatingCharts != null) profileRatingCharts.destroy();
    // var dt = 0;
    // for(var i=0; i<data.length; i++)
    // 	if(data[dt] < data[i])
    // 		dt = i;
    ...
    var tooltip = {
        ...
        formatter: function () {
            if (
                profileInfoDatas[3] == undefined ||
                profileInfoDatas[3][this.x] == undefined
            )
                return "";
            var val = profileInfoDatas[1][profileInfoDatas[3][this.x]];
            return (
                `<b><span info="Rank">${languageOption.general.Rank}</span>: ` +
                val.rank +
                " | " +
                `<span info="Rating">${languageOption.general.Rating}</span>: ` +
                val.oldRating +
                " >> " +
                val.newRating +
                "</b><br/>" +
    ...
    var plotOptions = {
        ...
        series: {
            ...
            events: {
                click: function (e) {
                    if (profileInfoDatas[3][e.point.x] == undefined) return;
                    var val =
                        profileInfoDatas[1][profileInfoDatas[3][e.point.x]];
                    $("[for=singleContent]").click();
                    loadSingleInformation(
                        0,
                        val.handle,
                        val.contestId,
                        new Date(),
...
const profileSubmissionFuncs = [
    function (q, argv) {
        ...
        if (Q.length < P.length) Q += "0".repeat(P.length - Q.length);
    ...
    function (q, argv) {
        ...
        var T = argv[1].toLowerCase();
        if (T.length <= String(q.problem.contestId).length) return false;
        return t.substring(0, T.length) == T;
    },
    function (q, argv) {
...
function problemSubmissionFilter(q, st, fi) {
    ...
    for (var i = 0; i < st.length; i++) {
        ...
        if (!flg) norFilter.push(s.toLowerCase());
    }
    var ret = [];
    for (var i = 0; i < q.length; i++) {
        var t = q[i];
...
function profileRefreshSubmissionListPages() {
    ...
    var L = Math.max(
        1,
        Math.floor(
            (profileFilteredSubmission.length +
                profilePageSubmissionCount -
                1) /
                profilePageSubmissionCount
        )
    );
    ...
    for (var i = l; i <= r; i++) {
        ...
        p += `<div style="font-size: 14px; text-align: center">${tm.pattern(
            "yyyy-MM-dd"
        )}<br/>${tm.pattern("hh:mm:ss")}</div>`;
        ...
        p += `<div><span style="cursor: pointer" onclick='openSubmission(${
            t.contestId
        }, ${
            t.id
        })'><div style='display: inline-block; width: 20px; text-align: center'>${getSubmissionIcon(
            t.verdict
        )}</div><div style='width: 50px; text-align: center; display: inline-block'> ${toColoredSubmissionInfo(
            t.verdict
        )}</div></span></div>`;
        p += `<div style="min-width: 70px">${toSmallTestset(t.testset)}${
            t.verdict == "OK"
                ? ""
        ...
        p += `<div style="font-size: 14px; min-width: 100px; text-align: center">${
            t.timeConsumedMillis
        }ms<br/>${Math.floor(t.memoryConsumedBytes / 1024)}KB</div>`;
        p += "</div>";
...
function profileRefreshSubmissionListDisplayer() {
    ...
    profileFilteredSubmission = problemSubmissionFilter(
        d,
        $(".profileSubmissionsInput").val(),
        $(".profileContestantOnlyIf").hasClass("selected")
    );
...
function profileDrawSubmissionFrame(q) {
    var l = q[1],
        ...
        d = q[3];
    var tm = new Date(
        l * miliSecondsPerDay + new Date().getTimezoneOffset() * 60 * 1000
    );
    var dt = tm.getDay();
    var DT = dt;
    var col = 0;
    ...
    var months = {};
    for (var i = 0; i < 7; i++)
        $(".profileSubmissionGraph").append(
            `<span style="transform: translate(-100%, -50%); position: absolute; top: ${
                10 + 20 + i * 16 + 8
            }px; left: ${15 + 19}px; color: grey">${localize("day" + i)}</span>`
        );
    for (var i = l; i <= r; i++) {
        var D = d[i - l];
        var mt = tm.getMonth();
        ...
        if (months[p] == undefined) months[p] = [mt, 100000, 0];
        months[p][1] = Math.min(months[p][1], col);
        months[p][2] = Math.max(months[p][2], col);
        var R = $(
            `<div class="profileSubmissionGraphItem" id="${
                i - l
            }" hover-text="${tm.pattern(
                "yyyy-MM-dd"
            )} : ${D}" style="z-index: 2; height: 14px; width: 14px; overflow: hidden; position: absolute; background: var(--profile-page-submission-light-background-color); top: ${
                10 + 20 + 1 + 16 * dt
            }px; left: ${15 + 20 + 2 + 16 * col}px; border-radius: 2px; ${
                new Date().getTime() < tm.getTime() ? "opacity: 0.6; " : ""
            }"></div>`
        );
        if (D != 0) {
            var op = Math.min(1, 0.3 + (D / 30.0) * 0.7);
            R.append(
                `<div style="background-color: var(--profile-page-submission-light-color); opacity: ${op}; width: 100%; height: 100%"></div>`
            );
        }
    ...
    for (var x in months)
        if (months.hasOwnProperty(x)) {
            var t = months[x];
            if (t[2] - t[1] <= 1) continue;
            var L = t[1] * 16 + 15 + 20 + 2;
            var R = (t[2] + 1) * 16 + 15 + 20 + 2;
            $(".profileSubmissionGraph").append(
                `<span style="transform: translate(-50%, -100%); position: absolute; top: ${
                    10 + 18
                }px; left: ${(L + R) / 2}px; color: grey">${localize(
                    "month" + t[0]
                )}</span>`
            );
        }
    $(".profileSubmissionGraph").append(
        `<div class='profileSubmissionPoptip closed'><span></span><div class='profileSubmissionPoptipTail'></div></div>`
    );
    $(".profileSubmissionGraphItem")
        .unbind("mouseover")
        .mouseover(function () {
            var T = $(this);
            ...
            var L = 15 + 20 + 2 - 10,
                R = 15 + 20 + 2 + 16 * 52 + 10;
            var rL = 0;
            $(".profileSubmissionPoptip").css(
                "left",
                (rL = L + ((lf - L) / (R - L)) * (R - L - 170 + 14))
            );
    ...
    function refreshSelectedBlocks(l, r, L, R) {
        ...
        function add(x) {
            var l = Math.floor((x + DT) / 7),
                r = Math.max((x + DT) % 7);
            $(".profileSubmissionGraph").append(
                `<div class='profileSubmissionGraphSelector' id=${x} style="top: ${
                    10 + 20 + 1 + r * 16
                }px; left: ${15 + 20 + 1 + 16 * l}px"></div>`
            );
        }
        if (r == -1) {
            for (var i = L; i <= R; i++) add(i);
        } else if (R == -1) {
            for (var i = l; i <= r; i++) del(i);
        } else {
            for (var i = l; i < L; i++) del(i);
            for (var i = r; i > R; i--) del(i);
            for (var i = L; i < l; i++) add(i);
            for (var i = R; i > r; i--) add(i);
        }
    }
    $(".profileSubmissionGraphItem")
        .unbind("mousedown")
        .mousedown(function (e) {
            if (sR != -1) {
                ...
                $(".profileSubmissionsInput").val(
                    `:time>=${timeList[sL]} :time<=${timeList[sR]}`
                );
                (sL = 0), (sR = -1);
            }
            ...
            $("body")
                ...
                .mousemove(function (e) {
                    ...
                    var p = (x - oL) / 16;
                    var q = (y - oT) / 16;
                    if (x < cX) p = Math.min(Math.floor(p), ll);
                    else p = Math.max(Math.floor(p), ll);
                    if (y < cY) q = Math.min(Math.floor(q), rr);
                    else q = Math.max(Math.floor(q), rr);
                    var tar = p * 7 + q;
                    var pL = sL,
                        pR = sR;
...
function profileReloadSubmissionsPageInfo(d, day = 0) {
    ...
    for (var i = 0; i < d.length; i++) {
        ...
        if (appYearIf[y] == undefined) appYearIf[y] = [];
    ...
    function getStatistics(l, r, ds) {
        var ret = [];
        for (var i = l; i <= r; i++) ret.push(0);
        for (var i = 0; i < ds.length; i++) {
            var q = Math.floor(
                (ds[i].creationTimeSeconds -
    ...
    function getDateLenghts(nd, info) {
        var r = Math.floor(
            (nd.getTime() - new Date().getTimezoneOffset() * 60 * 1000) /
                miliSecondsPerDay
        );
        var l = r;
        var dt = nd.getDay();
        ...
        var L =
            l * miliSecondsPerDay + new Date().getTimezoneOffset() * 60 * 1000;
        var R =
            (r + 1) * miliSecondsPerDay +
            new Date().getTimezoneOffset() * 60 * 1000;
        for (var i = 0; i < d.length; i++)
            if (
                d[i].creationTimeSeconds * 1000 >= L &&
                d[i].creationTimeSeconds * 1000 < R
            )
                rr.push(d[i]), ++cnt;
    ...
    profileSubmissionTimeList.push(
        getDateLenghts(new Date(), localize("recent"))
    );
    $(".profileSubmissionGraphSelection").append(
        `<span class='profileSubmissionGraphSelectionBlock selected' sid="${0}">${localize(
            "recent"
        )}<span style='font-size: 9px'>:${cnt}</span></span>`
    );
    for (var i = maxD; i >= minD; i--)
        if (appYearIf[i] != undefined)
            profileSubmissionTimeList.push([
                ...
                getDateFirst(i),
                getDateLast(i),
                getStatistics(getDateFirst(i), getDateLast(i), appYearIf[i]),
            ]),
                $(".profileSubmissionGraphSelection").append(
                    `<span class='profileSubmissionGraphSelectionBlock' sid="${
                        profileSubmissionTimeList.length - 1
                    }">${i}<span style='font-size: 9px'>:${
                        appYearIf[i].length
                    }</span></span>`
                );
    delete appYearIf;
    appYearIf = {};
    $(".profileSubmissionGraphSelectionBlock")
        .unbind("click")
        .click(function () {
            var t = $(this).attr("sid");
            $(".profileSubmissionGraphSelectionBlock.selected").removeClass(
                "selected"
            );
            $(this).addClass("selected");
...
$(".profileContestantOnlyIf").click(function () {
    ...
    if (profileFilteredSubmission == undefined) return;
    profileRefreshSubmissionListEnter();
});
...
function infoLoadUsername(un) {
    ...
    for (var i = 0; i <= 2; i++)
        if (profileInfoLoaders[i] != null) profileInfoLoaders[i].abort();
    ...
    function infoLoadData(id, url, S) {
        ...
        profileInfoLoaders[id] = $.ajax({
            ...
            success: function (json) {
                ...
                if (un + "#" + hCode != profileInfoCurrentAsked) return;
                if (json.status != "OK")
                    $(".infoLoaders" + id).html(
    ...
    infoLoadData(
        0,
        generateAuthorizeURL(settings.codeforcesApiUrl + "/user.info", {
            handles: un,
        }),
        function (d) {
            d = d[0];
            ...
            $(".infoSmallUserRanks").html(
                `<div class='${ratingToClass(d.rating)}'>${ratingToGrade(
                    d.rating
                )}</div>`
            );
            $(".infoUsername").html(
                `<div class='${ratingToClass(d.rating)}'>${d.handle}</div>`
            );
            $(".infoUserRanksValue").html(
                `<div style='display: inline-block' class='${ratingToClass(
                    d.rating
                )}'>${ratingToSmalln(d.rating)} ${
                    d.rating == undefined ? "0" : d.rating
                }</div> (max. <div style='display: inline-block' class='${ratingToClass(
                    d.maxRating
                )}'>${ratingToSmalln(d.maxRating)} ${
                    d.maxRating == undefined ? "0" : d.maxRating
                }</div>)`
            );
            $(".infoUserContributionValue").html(
                `${
                    d.contribution == 0
                        ? 0
                        : d.contribution > 0
                        ? `<span class='green'>+${d.contribution}</span>`
                        : `<span class='red'>${d.contribution}</span>`
                }`
            );
            $(".infoUserFriends > span")
                .attr("argv", `[${d.friendOfCount}]`)
                .html(
                    languageOption.general.infoFriends.format([d.friendOfCount])
                );
            $(".infoUserTimes > div:first-child").html(
                new Date(d.registrationTimeSeconds * 1000).pattern(
                    "yyyy-MM-dd hh:mm"
                )
            );
            $(".infoUserTimes > div:last-child").html(
                new Date(d.lastOnlineTimeSeconds * 1000).pattern(
                    "yyyy-MM-dd hh:mm"
                )
            );
        }
    ...
    infoLoadData(
        1,
        generateAuthorizeURL(settings.codeforcesApiUrl + "/user.rating", {
            handle: un,
        }),
        function (d) {
            ...
            $(".infoBlockTitle > div")
                .attr("argv", `[${d.length}]`)
                .html(
                    languageOption.general.infoContestTitle.format([d.length])
                );
            ...
            for (var i = 0; i < d.length; i++)
                D.push([d[i].ratingUpdateTimeSeconds * 1000, d[i].newRating]),
                    (profileInfoDatas[3][d[i].ratingUpdateTimeSeconds * 1000] =
                        i);
    ...
    infoLoadData(
        2,
        generateAuthorizeURL(settings.codeforcesApiUrl + "/user.status", {
            handle: un,
        }),
        function (d) {
            ...
            function dec(x) {
                return (
                    ...
                    `<span style="font-size: 24px">${x.substring(
                        x.length - 3
                    )}</span>`
                );
            }
...
function infoSetupUsername() {
    ...
    if (val == "") val = currentLoginHandle;
    if (val.length < 3 || val.length > 24 || !queryUsrename.test(val)) {
        $(".infoChangeUsernameInputArea > button")
            .removeClass("primaryColor")
            .addClass("dangerColor");
        $(".infoChangeUsernameInputArea > button").html(
            "<span class='fas fa-exclamation-triangle'></span>"
        );
        setTimeout(function () {
            $(".infoChangeUsernameInputArea > button")
                .addClass("primaryColor")
                .removeClass("dangerColor");
            $(".infoChangeUsernameInputArea > button").html(
...
$(".infoChangeUsernameInputArea > input").bind("keydown", function (event) {
    if (event.keyCode == "13") infoSetupUsername();
});
