...
function getRealTimeLimit(x) {
    x = x.split(" ");
    if (x[1] == "minute" || x[1] == "minutes" || x[1] == "m") return x[0] + "m";
    if (x[1] == "second" || x[1] == "seconds" || x[1] == "s") return x[0] + "s";
    return x[0] + "ms";
}
...
function getRealMemoryLimit(x) {
    x = x.split(" ");
    if (x[1] == "megabytes" || x[1] == "megabyte") return x[0] + "MB";
    if (x[1] == "gigabytes" || x[1] == "gigabyte") return x[0] + "GB";
    return x[0] + "KB";
}

function getRealMemoryLimitNumber(x) {
    x = x.split(" ");
    if (x[1] == "megabytes" || x[1] == "megabyte") return Number(x[0]);
    if (x[1] == "gigabytes" || x[1] == "gigabyte") return Number(x[0]) * 1024;
    return Number(x[0]) / 1024;
}

function getRealInputFile(x) {
    if (x == "standard input") return "stdin";
    return x;
}

function getRealOutputFile(x) {
    if (x == "standard output") return "stdout";
    return x;
}
...
function initProblemPageInfo(page, data, id) {
    ...
    if (data.find(".time-limit").length != 0)
        tg.append(
            `<div class='problemTag primaryColor'>
                ...
                ${getRealTimeLimit(
                    data.find(".time-limit").contents().eq(1).text()
                )}
            </div>`
        ),
            (problemCurrentPageList[id][4].timelimit = getRealTimeLimitNumber(
                data.find(".time-limit").contents().eq(1).text()
            ));
    if (data.find(".memory-limit").length != 0)
        tg.append(
            `<div class='problemTag warningColor'>
                ...
                ${getRealMemoryLimit(
                    data.find(".memory-limit").contents().eq(1).text()
                )}
            </div>`
        ),
            (problemCurrentPageList[id][4].memoryLimit =
                getRealMemoryLimitNumber(
                    data.find(".memory-limit").contents().eq(1).text()
                ));
    if (data.find(".input-file").length != 0)
        tg.append(
            `<div class='problemTag successColor'><i class='fas fa-sign-in-alt'></i>${getRealInputFile(
                data.find(".input-file").contents().eq(1).text()
            )}</div>`
        ),
            (problemCurrentPageList[id][4].input = getInputFileType(
                getRealInputFile(
                    data.find(".input-file").contents().eq(1).text()
                )
            ));
    if (data.find(".output-file").length != 0)
        tg.append(
            `<div class='problemTag dangerColor'><i class='fas fa-sign-out-alt'></i>${getRealOutputFile(
                data.find(".output-file").contents().eq(1).text()
            )}</div>`
        ),
            (problemCurrentPageList[id][4].output = getOutputFileType(
                getRealOutputFile(
                    data.find(".output-file").contents().eq(1).text()
                )
            ));
    ...
    for (var i = 0; i < qq.length; i++) {
        if (
            qq.eq(i).attr("class") != "header" &&
            qq.eq(i).attr("class") != "sample-tests"
        ) {
            var l = "Description";
            ...
            var oo = $(
                `<div class="blockManager"><div class="blockManagerTitle">Samples</div></div>`
            );
            oo.append(o);
            page.append(oo);
        }
    ...
    problemNewWinJQ.append(
        `<script>reloadMath("problem-${problemCurrentPageList[id][0]}")</script>`
    );
}
...
function loadProblem(x, info) {
    ...
    if (
        problemCurrentPageList.find(function (q) {
            return q[0] == x;
        }) == undefined
    )
        return;
    ...
    problemNewWinJQ
        .find(`.innerContent > [problem-id=${x}]`)
        .html(
            `<div style="display: grid; place-items: center; width: 100%; height: 100%"><i class="fas fa-sync-alt fa-spin fa-3x"></i></div>`
        );
    problemCurrentPageList[p][2] = 1;
    tmpSideBarItem
        .eq(p)
        .html(
            `${loadType[problemCurrentPageList[p][2]](x)}${
                problemCurrentPageList[p][0]
            }<div class="closeCurrentProblemPage" id=${p}><i class="fas fa-times"></i></div>`
        );
    tmpSideBarItem.unbind("click").click(function () {
        ...
        problemNewWinJQ
            .find(".problemName")
            .html(problemCurrentPageList[problemFocusOn][0]);
        problemNewWinJQ.find(".innerContent > div").css("display", "none");
        problemNewWinJQ
            .find(
                `.innerContent > [problem-id=${problemCurrentPageList[problemFocusOn][0]}]`
            )
            .css("display", "block");
    });
    problemNewWinJQ
        .find(".closeCurrentProblemPage")
        .unbind("click")
        .click(function () {
            event.stopPropagation();
    ...
    if (info != undefined) {
        ...
        problemCurrentPageList[p][4].contestName = $(info)
            .find(".caption")
            .html();
        initProblemPageInfo(
            problemNewWinJQ.find(`.innerContent > [problem-id=${x}]`),
            $(info).find(
                `.problemindexholder[problemindex=${
                    getProblemIndexes(x)[1]
                }] .problem-statement`
            ),
            p
        );
    ...
    problemCurrentPageList[p][1] = $.ajax({
        url:
            settings.mainURL +
            `/${getProblemIndexes(x)[0] >= 100000 ? "gym" : "contest"}/` +
            getProblemIndexes(x)[0] +
            "/problem/" +
            getProblemIndexes(x)[1] +
            "?locale=en",
        success: function (data) {
            if (
                data.indexOf(`data-entityId="${getProblemIndexes(x)[0]}"`) == -1
            ) {
                if (
                    problemCurrentPageList.find(function (q) {
                        return q[0] == x;
                ...
                problemNewWinJQ
                    .find(`.innerContent > [problem-id=${x}]`)
                    .html(
                        `<div style="display: grid; place-items: center; width: 100%; height: 100%"><i class="fas fa-unlink red fa-3x"></i></div>`
                    );
                tmpSideBarItem.unbind("click").click(function () {
                    tmpSideBarItem.eq(problemFocusOn).removeClass("chosen");
                    problemFocusOn = Number($(this).attr("id"));
            ...
            problemCurrentPageList[p][4].contestName = $(data)
                .find("#sidebar")
                .eq(0)
                .find("a")
                .eq(0)
            ...
            initProblemPageInfo(
                problemNewWinJQ.find(`.innerContent > [problem-id=${x}]`),
                $(data).find(".ttypography > .problem-statement"),
                p
            );
            problemCurrentPageList[p][2] = 0;
            tmpSideBarItem = problemNewWinJQ.find(".sideBarItem");
    ...
    if (gid == undefined) {
        for (var i = 0; i < x.length; i++) {
            ...
            problemNewWinJQ
                .find(".innerContent")
                .append(
                    `<div class="innerContentPage" id="problem-${x[i]}" problem-id="${x[i]}" style="display: none"><div style="display: grid; place-items: center; width: 100%; height: 100%"><i class="fas fa-sync-alt fa-spin fa-3x"></i></div></div>`
                );
        ...
        for (var i = 0; i < problemCurrentPageList.length; i++)
            if (getProblemIndexes(problemCurrentPageList[i][0])[0] == gid)
                dL.push(i);
        while (dL.length != 0) killProblemListItem(dL.pop());
        problemGroupRange[gid] = [
            problemCurrentPageList.length,
            x.length,
            true,
        ];
        ...
        problemCurrentPageList.splice(
            problemGroupRange[gid][0],
            problemGroupRange[gid][1]
        );
        ...
        problemFocusOn = problemGroupRange[gid][0];
        for (var i = 0; i < x.length; i++) {
            problemCurrentPageList.splice(problemGroupRange[gid][0] + i, 0, [
                x[i],
                null,
                1,
                [[], []],
            ...
            problemNewWinJQ
                .find(`.innerContent > [problem-id=${x[i]}]`)
                .remove();
            problemNewWinJQ
                .find(".innerContent")
                .append(
...
function killProblemListItem(x) {
    ...
    problemNewWinJQ
        .find(`.innerContent > [problem-id=${problemCurrentPageList[x][0]}]`)
        .remove();
    if (problemCurrentPageList[x][1] != null)
        problemCurrentPageList[x][1].abort();
    if (problemCurrentPageList.length - 1 == x) problemCurrentPageList.pop();
    else problemCurrentPageList.splice(x, 1);
    if (problemFocusOn >= x) --problemFocusOn;
    if (problemFocusOn < 0) problemFocusOn = 0;
    for (var t in problemGroupRange)
        if (problemGroupRange.hasOwnProperty(t))
            if (problemGroupRange[t][0] > x) problemGroupRange[t][0]--;
    if (problemGroupRange[q] != undefined) {
        var u = problemGroupRange[q];
        if (u[0] <= x && u[0] + u[1] > x) {
            --problemGroupRange[q][1];
            if (problemGroupRange[q][1] == 0) delete problemGroupRange[q];
        }
    }
}
var loadTypes = [
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                q.find(".problemindexholder").each(function () {
                    ret.push(
                        String(cid) + $.trim($(this).attr("problemindex"))
                    );
                });
                S(ret, data);
            },
...
function flushProblemNewWin() {
    ...
    for (var i = 0; i < problemCurrentPageList.length; i++) {
        ...
        if (
            problemGroupRange[cid] != undefined &&
            problemGroupRange[cid][0] == i
        ) {
            var curr = $("<div class='sideBarGroupContents'></div>");
            if (problemGroupRange[cid][2]) curr.css("padding-bottom", "10px");
            if (lastContestFolder) curr.css("margin-top", "10px");
            var T = `<div class="sideBarGroup" id="${cid}">${
                problemGroupRange[cid][2]
                    ? "<i class='fas fa-folder-open'></i>"
                    : "<i class='fas fa-folder'></i>"
            }${cid}<div class="closeCurrentGroup" id=${cid}><i class="fas fa-times"></i></div></div>`;
            for (var j = 0; j < problemGroupRange[cid][1]; j++, i++)
                T += `<div class="sideBarItem" id="${i}" ${
                    problemGroupRange[cid][2] ? "" : " style='display: none'"
                }>${loadType[problemCurrentPageList[i][2]](
                    problemCurrentPageList[i][0]
                )}${
                    problemCurrentPageList[i][0]
                }<div class="closeCurrentProblemPage" id=${i}><i class="fas fa-times"></i></div></div>`;
            curr.append(T);
            --i;
        ...
        tmpField.append(
            `<div class="sideBarItem" id="${i}">${loadType[
                problemCurrentPageList[i][2]
            ](problemCurrentPageList[i][0])}${
                problemCurrentPageList[i][0]
            }<div class="closeCurrentProblemPage" id=${i}><i class="fas fa-times"></i></div></div>`
        );
    ...
    tmpField.append(
        `<div class="addProblemSidebar"><i class='fas fa-add'></i><span info='add'>${languageOption.general.add}</span></div>`
    );
    let tmpSideBarItem = problemNewWinJQ.find(".sideBarItem");
    ...
    problemNewWinJQ
        .find(".closeAddProblemPage")
        .unbind("click")
        .click(function () {
            problemNewWinJQ.find(".addProblemWindow").css("opacity", "0");
            setTimeout(function () {
                problemNewWinJQ
                    .find(".addProblemWindow")
                    .css("display", "none");
            }, 500);
        });
    problemNewWinJQ
        .find(".addProblemSidebar")
        .unbind("click")
        .click(function () {
    ...
    problemNewWinJQ
        .find(".problemInfoInputArea input")
        .unbind("keydown")
    ...
    problemNewWinJQ
        .find(".contestInfoInputArea input")
        .unbind("keydown")
        .bind("keydown", function (event) {
            if (event.keyCode == "13") {
                tmpButton1.click();
...
function initProblemNewWin() {
    ...
    problemNewWinJQ
        .find("#ThemeTypeIf")
        .attr("href", DarkMode ? "./css/dark.css" : "./css/default.css");
    ...
    } else problemNewWinJQ.find(".submitCode").css("display", "inline");
    problemNewWinJQ
        .find(".reloadStatement")
        .unbind("click")
        .click(function () {
        ...
        .click(function () {
            ...
            for (var name in submissionLangs)
                if (submissionLangs.hasOwnProperty(name))
                    problemNewWinJQ
                        .find(".submitLanguageChoser")
                        .append(
                            `<option value=${name}>${submissionLangs[name]}</option>`
                        );
            problemNewWinJQ
                .find(".submitLanguageChoser")
                .val(settings.statementDefaultLanguage);
    ...
    problemNewWinJQ
        ...
        .click(function () {
            if (
                problemCurrentPageList.length <= problemFocusOn ||
                problemCurrentPageList[problemFocusOn][2] != 0
            )
                return;
            console.log("Posting...");
            ...
            for (var pr in CompetitiveCompanionPort)
                $.ajax({
                    url:
                        "http://localhost:" +
                        $.trim(CompetitiveCompanionPort[pr]) +
                        "/",
                    type: "POST",
                    dataType: "JSON",
                    ...
                    data: JSON.stringify({
                        ...
                        group: problemCurrentPageList[problemFocusOn][4]
                            .contestName,
                        url: `https://codeforces.com/${
                            getProblemIndexes(
                                problemCurrentPageList[problemFocusOn][0]
                            )[0] >= 100000
                                ? "gym"
                                : "contest"
                        }/${
                        ...
                        interactive: false,
                        memoryLimit:
                            problemCurrentPageList[problemFocusOn][4]
                                .memoryLimit,
                        timeLimit:
                            problemCurrentPageList[problemFocusOn][4].timelimit,
                        tests: sampleWrapper(
                            problemCurrentPageList[problemFocusOn][3][0],
                            problemCurrentPageList[problemFocusOn][3][1]
                        ),
                        input: problemCurrentPageList[problemFocusOn][4].input,
                        output: problemCurrentPageList[problemFocusOn][4]
                            .output,
                    }),
                });
        });
        ...
        .click(function () {
            ...
            function sender(pp, prr, cc) {
                setTimeout(function () {
                    $.ajax({
                        url:
                            "http://localhost:" +
                            $.trim(CompetitiveCompanionPort[prr]) +
                            "/",
                        type: "POST",
                        dataType: "JSON",
                        contentType: "application/json",
            ...
            for (var i = 0; i < problemCurrentPageList.length; i++) {
                ...
                var info = JSON.stringify({
                    ...
                    url: `https://codeforces.com/${
                        getProblemIndexes(problemCurrentPageList[i][0])[0] >=
                        100000
                            ? "gym"
                            : "contest"
                    }/${
                        ...
                        getProblemIndexes(problemCurrentPageList[i][0])[1]
                    }`,
                    interactive: false,
                    memoryLimit: problemCurrentPageList[i][4].memoryLimit,
                    timeLimit: problemCurrentPageList[i][4].timelimit,
                    tests: sampleWrapper(
                        problemCurrentPageList[i][3][0],
                        problemCurrentPageList[i][3][1]
                    ),
    ...
    problemNewWinJQ.find(".submitButton > button").click(function () {
        ...
        submitSolution(
            getProblemIndexes(problemCurrentPageList[problemFocusOn][0])[0],
            ...
            function (id, cid, gc) {
                ...
                tmpButton.html(
                    `<span info="submitSuccess">${languageOption.general.submitSuccess}</span>`
                );
                setTimeout(function () {
                    tmpButton
                        .addClass("primaryColor")
                        .removeClass("successColor");
                    tmpButton.html(
                        `<i class="fas fa-paper-plane"></i><span info="sendAnswer">${languageOption.general.sendAnswer}</span>`
                    );
                    ...
                    if (id != undefined)
                        problemNewWinJQ.append(
                            `<script>addWatcher('${id}', '${cid}', '${gc}')</script>`
                        );
                }, 1000);
            },
            function (x, y) {
